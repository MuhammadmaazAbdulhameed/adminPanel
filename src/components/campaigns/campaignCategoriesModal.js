...
const CampaignCategoriesModal = ({
  ...
  const setInputStateFunction = (e) => {
    setCampaignCategoryObject(prev => ({ ...prev, [e.target.name]: e.target.value }))
  }
  return (
    <Modal className='modal-dialog-centered px-5 ' isOpen={modalVisible} toggle={() => modalVisibilityAction(!modalVisible)}>
      ...
      <ModalBody className='px-sm-2 mx-50 pt-0'>
        <h3 className='text-center mb-1 text-black fw-bolder'>{editModal ? "Edit " : "Add New "} Category</h3>
        <Row tag='form' className='gy-1 gx-2 mt-75'>
          <Col xs={12}>
            <Input id="name" name="name"
              value={campaignCategoryObject.name}
